...
def _safe_palette(
    n: int, chart_name: str, max_cont: int = 30
) -> tuple[bool, list[str]]:
    """
...
def country_bar(
    df: pd.DataFrame, title_suffix: str = "", top_n: Optional[int] = 10
) -> go.Figure:
    """
...
def year_scatter(
    df: pd.DataFrame, title_suffix: str = "", interpolate: bool = False
) -> go.Figure:
    """
...
def year_line(
    ...
    fig = px.line(
        yearly, x="Year", y="Value", markers=True, title=f"GDP Over Time{title_suffix}"
    )
