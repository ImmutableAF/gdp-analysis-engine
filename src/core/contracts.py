...
class DataFrameLoadable(Protocol):
    """
    Protocol for any object that can load data from a source into a DataFrame.

    Any class with a matching load() signature satisfies this contract and
    can be passed wherever a DataFrameLoadable is expected.

    Methods
    -------
    load(source)
        Load data from a source identifier and return it as a DataFrame.
    ...
    ...
    >>> loader: DataFrameLoadable = CSVLoader()
    """

    def load(self, source: str) -> pd.DataFrame:
        """
        Load data from a source identifier into a DataFrame.

        Parameters
        ----------
        source : str
            Path, URL, or any identifier pointing to the data source.
...
class DataFrameOutput(Protocol):
    """
    Protocol for any object that can write a DataFrame to an output destination.

    Any class with a matching write() signature satisfies this contract.
    The destination (file, database, stream, etc.) is left entirely to
    ...
    ...
    >>> writer: DataFrameOutput = CSVWriter()
    """
